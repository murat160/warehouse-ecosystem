...
export interface PageHeaderProps {
  ...
  subtitle?: string;
  /**
   * - `true` — показать стрелку назад (navigate(-1), либо `/` если истории нет)
   * - `false` — скрыть
   * - не задано — показать автоматически на любой странице, кроме `/`
   * - строка — навигация на конкретный путь
   */
  back?: boolean | string;
  right?: ReactNode;
  actions?: ReactNode;
...
export function PageHeader({ title, subtitle, back, right, actions, badge, roleNote }: PageHeaderProps) {
  const nav = useNavigate();
  const loc = useLocation();
  const showBack =
    back === false ? false :
    back === true || typeof back === 'string' ? true :
    loc.pathname !== '/';
    ...
    <div className="bg-[#1F2430] text-white px-5 pt-4 pb-8 rounded-b-3xl">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {roleNote && (
            <p className="text-[11px] text-white/50 mt-1" style={{ fontWeight: 600 }}>
              {roleNote}
            </p>
          )}
        </div>
